...
const generateBarcode = async (opt: BarcodeOptions) => {
  ...
  const sanitizedAthleteName =
    opt.athleteName && opt.athleteName.trim().length > 0
      ? opt.athleteName.trim()
      : "Unknown";

  console.log(
    `Sanitation details: ${JSON.stringify({
...
const listener = async (
  ...
  const parameters = new URL(
    request.url ?? "",
    `https://${request?.headers.host ?? "example.com"}`,
  ).searchParams;
